...
const tests = [
  'admin882nzggzz!fortress-auth-salt',
  'hello world',
  '',
...
for (const t of tests) {
  const exp = crypto.createHash('sha256').update(t).digest('hex');
  const act = sha256Hex(t);
  console.log(`Test '${t.slice(0, 10)}...':`, exp === act ? 'PASS' : `FAIL (exp: ${exp}, act: ${act})`);
}
