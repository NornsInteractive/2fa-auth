...
interface ProviderPickerModalProps {
  ...
  onSelect: (providerName: string, iconType?: string) => void;
  onClose: () => void;
}
...
export const ProviderPickerModal: React.FC<ProviderPickerModalProps> = ({
  ...
  const handleCreateAndSelect = async () => {
    ...
    const created = await addProvider({ name: newProviderName.trim(), icon: 'shield', color: palette.primary });
    setNewProviderName('');
    setShowAddForm(false);
    onSelect(created.name, created.icon);
    onClose();
  };
        ...
        >
          ...
          <View style={styles.header}>
            <View style={styles.headerTitleRow}>
              <Icon name="hub" size={20} color={palette.primary} />
          ...
          <View style={styles.searchBox}>
            ...
            >
              ...
              <TextInput
                ...
                placeholder={language === 'zh' ? '输入模糊搜索提供商...' : 'Search providers...'}
          ...
          {showAddForm ? (
            <View style={[styles.createForm, { backgroundColor: palette.surfaceContainerLow }]}>
              ...
              <View style={styles.createRow}>
                <TextInput
                  ...
                  placeholder={language === 'zh' ? '输入提供商名称' : 'Provider Name'}
                  placeholderTextColor={palette.outline}
            ...
            >
              ...
              <Text style={[styles.addNewTriggerText, { color: palette.primary }]}>
                {language === 'zh' ? '+ 新建自定义提供商' : '+ Add Custom Provider'}
              </Text>
            </TouchableOpacity>
          )}
          ...
          <ScrollView style={styles.list} showsVerticalScrollIndicator={false}>
            {filteredProviders.length > 0 ? (
              filteredProviders.map((p) => {
                const isSelected =
                  selectedProviderName.toLowerCase().trim() === p.name.toLowerCase().trim();
                ...
                return (
                  ...
                  >
                    <View style={styles.itemLeft}>
                      ...
                      >
                        <Icon name={p.icon || 'shield'} size={18} color="#ffffff" fill />
                      </View>
                      <Text
                        style={[
                          styles.itemName,
              ...
              <View style={styles.emptyBox}>
                <Text style={[styles.emptyText, { color: palette.onSurfaceVariant }]}>
                  {language === 'zh' ? '未找到相关提供商' : 'No providers found'}
                </Text>
                <TouchableOpacity
                  onPress={() => {
                    setNewProviderName(searchQuery);
                    setShowAddForm(true);
                  }}
                ...
                >
                  <Text style={styles.quickAddBtnText}>
                    {language === 'zh'
                      ? `直接添加 "${searchQuery}" 为提供商`
                      : `Add "${searchQuery}" as new provider`}
                  </Text>
                </TouchableOpacity>
              </View>
            )}
